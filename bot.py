...
SOL_USD_RATE = float(os.getenv("SOL_USD_RATE", "0"))
...
TRIAL_ENABLED = True  # 5-day free trial restored.
...
class PaidAccessMiddleware(BaseMiddleware):
    ...
    """
    PUBLIC_COMMANDS = {"/start", "/premium", "/verify", "/trial"}
    PUBLIC_CALLBACKS = {"premium", "trial"}
...
def normalize_database_url(url):
    """Normalize common Render/Supabase DATABASE_URL formatting mistakes."""
    if not url:
        return ""
    value = url.strip().strip('\"').strip("'")
    # Prevent accidental DATABASE_URL=DATABASE_URL=... values.
    if value.startswith("DATABASE_URL="):
        value = value.split("=", 1)[1].strip().strip('\"').strip("'")
    # asyncpg expects postgres:// or postgresql://, not SQLAlchemy's +asyncpg form.
    value = value.replace("postgresql+asyncpg://", "postgresql://", 1)
    value = value.replace("postgres+asyncpg://", "postgres://", 1)
    # Recover from an accidentally duplicated scheme.
    while value.startswith("postgresql://postgresql://"):
        value = "postgresql://" + value[len("postgresql://postgresql://"): ]
    while value.startswith("postgres://postgres://"):
        value = "postgres://" + value[len("postgres://postgres://"): ]
    return value
...
async def is_premium(user_id):
    # Admins always have unrestricted/free access to every premium feature.
...
async def payment_monitor_loop():
    if not PAYMENT_WALLET:
        logging.warning("Automatic payment monitor disabled: PAYMENT_WALLET missing.")
        return
    seen=set()
    while True:
        if DB is None:
            logging.warning("Payment monitor waiting for PostgreSQL; it will start automatically after DATABASE_URL becomes available.")
            await asyncio.sleep(max(15, int(os.getenv("DB_RETRY_SECONDS", "30"))))
            continue
        try:
...
async def send_free_daily():
    ...
    for mint in WATCH_TOKENS:
        # Prefer the same live market-data path used by premium alerts so the
        # free channel also gets a real current price when provider data exists.
        try:
            live = await build_live_alert(mint)
        except Exception:
            live = None
    ...
    live_price_line = (f"💵 Live price: <b>{price(candidate['price'])}</b>\n"
                      if candidate.get("price") is not None else
                      "💵 Live price: <b>Unavailable</b>\n")
    text = (
        "🟢 <b>FREE DAILY SIGNAL</b>\n\n"
        + address_line
        + f"📌 {candidate.get('symbol', 'TOKEN')}\n"
        + live_price_line
        + f"🛡️ Safety score: <b>{candidate['safety_score']}/100</b>\n\n"
        "⚠️ Market data can be delayed or unavailable.\n\n"
        "⭐ <b>Premium</b> adds the full live scanner and intelligence feed."
    )
...
def build_start_keyboard(user_id, premium=False):
    ...
    if premium or user_id in ADMIN_IDS:
        ...
        rows = [
            [InlineKeyboardButton(text="🎁 5-Day Free Trial", callback_data="trial")],
            [InlineKeyboardButton(text="💎 Unlock Premium", callback_data="premium")]
        ]
    if user_id in ADMIN_IDS:
...
async def get_sol_usd_price():
    """Return a live SOL/USD price with provider fallback and short caching."""
    now = time.time()
    cache = getattr(get_sol_usd_price, "_cache", {"price": None, "ts": 0})
    if cache["price"] and now - cache["ts"] < 60:
        return cache["price"]

    # Primary provider configured by SOL_USD_PRICE_URL (Binance by default).
    try:
        status, data = await _http_json_request("GET", SOL_USD_PRICE_URL)
        if status == 200 and isinstance(data, dict):
            p = float(data.get("price"))
            if p > 0:
                get_sol_usd_price._cache = {"price": p, "ts": now}
                return p
    except Exception:
        pass

    # Free fallback provider. This prevents a temporary Binance failure from
    # showing a stale hard-coded SOL value in the payment screen.
    try:
        status, data = await _http_json_request(
            "GET", "https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd"
        )
        if status == 200 and isinstance(data, dict):
            p = float((data.get("solana") or {}).get("usd"))
            if p > 0:
                get_sol_usd_price._cache = {"price": p, "ts": now}
                return p
    except Exception:
    ...
    # Never invent a price. A configured SOL_USD_RATE may be used only as an
    # explicit deployment fallback; the default is 0 so it cannot be stale by accident.
...
async def trial_command(message: Message):
    if message.from_user.id in ADMIN_IDS:
        await message.answer("👑 Admin access is free and unrestricted.", parse_mode="HTML")
        return
    ok, text = await activate_trial(message.from_user.id, message.from_user.username)
...
async def trial_callback(callback: CallbackQuery):
    if callback.from_user.id in ADMIN_IDS:
        await callback.message.answer("👑 Admin access is free and unrestricted.", parse_mode="HTML")
    else:
        ok, text = await activate_trial(callback.from_user.id, callback.from_user.username)
...
# ============================================================
# CHANNEL DELIVERY WATCHDOG
# ============================================================
async def channel_delivery_watchdog():
    """Keep configured channels receiving a real live market message.

    This is only a delivery fallback: normal free/premium signal loops still
    run unchanged. A fallback is sent only when a channel has been quiet for
    the configured period, so it does not flood the channels.
    """
    last_free = 0.0
    last_paid = 0.0
    interval = max(300, int(os.getenv("CHANNEL_WATCHDOG_SECONDS", "300")))
    quiet_after = max(3600, int(os.getenv("CHANNEL_QUIET_SECONDS", "21600")))
    while True:
        try:
            now = time.time()
            sol = await get_sol_usd_price()
            if sol is not None:
                text = (
                    "📡 <b>LIVE SOLANA MARKET UPDATE</b>\n\n"
                    f"💎 SOL: <b>{price(sol)}</b>\n"
                    f"🕒 Updated: <b>{datetime.now(timezone.utc):%d %b %Y %H:%M UTC}</b>\n\n"
                    "⚠️ Live market data is informational and can change quickly."
                )
                if FREE_CHANNEL_ID and now - last_free >= quiet_after:
                    await BOT.send_message(FREE_CHANNEL_ID, text, parse_mode="HTML")
                    last_free = now
                if PREMIUM_CHANNEL_ID and now - last_paid >= quiet_after:
                    await BOT.send_message(PREMIUM_CHANNEL_ID, text, parse_mode="HTML")
                    last_paid = now
        except Exception:
            logging.exception("Channel delivery watchdog failed. Check that the bot is an admin in both channels and that channel IDs are correct.")
...
async def main():
    ...
    tasks = [
        ...
        asyncio.create_task(channel_delivery_watchdog()),
